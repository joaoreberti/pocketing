// Lifted from
// https://github.com/danburzo/percollate/blob/b65f6748d2c5e1f2e2380584e8ad85e354e7763f/src/enhancements.js
//@ts-nocheck
import { JSDOM } from "jsdom";
import { parseSrcset, stringifySrcset } from "srcset";
import replaceElementType from "./replace-element-type";
import { isImageURL } from ".//file-mimetype";

export const enhancePage = function (dom: JSDOM) {
  ...
  Array.from(
    ...
    ["src", "srcset", "lazySrc", "lazySrcset"].forEach((attr) => {
      if (attr in img.dataset) {
        img.setAttribute(attr.replace(/^lazyS/, "s"), img.dataset[attr]);
      }
    });
  });
...
function singleImgToFigure(doc) {
  Array.from(doc.querySelectorAll("img:only-child")).forEach((image) => {
    /*
		 ...
		 */
    const content = image.parentNode.tagName === "A" ? image.parentNode : image;
